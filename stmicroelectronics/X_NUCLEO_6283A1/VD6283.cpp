...
static void setup_device(struct VD6283_device *dev, DevI2C *pClient, uint8_t dev_7bits_i2c_addr, void *hdl)
{
	memset(dev, 0, sizeof(*dev));
	dev->client = pClient;
    dev->dev_7bits_i2c_addr = dev_7bits_i2c_addr;
	dev->hdl = hdl;
	dev->st = DEV_INIT;
...
static STALS_ErrCode_t check_supported_device(struct VD6283_device *dev)
{
	...
	res = STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_DEVICE_ID, &dev->device_id);
	if (res) return res;
	
    res = STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_REVISION_ID, &dev->revision_id);
	if (res) return res;

	if (is_cut_valid(dev)) return STALS_NO_ERROR;
	...
	uint8_t *is_data_ready)
{
	...
	res = STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_IRQ_CTRL_ST, &isr_ctrl_st);
	...
	uint8_t data)
{
	...
	res = STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_MODE, &ac_mode);
	if (res)
		return res;
	ac_mode = (ac_mode & ~mask) | (data & mask);
	return STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_MODE, ac_mode);
}
	...
	uint32_t *measure)
{
	...
	res = STALS_RdMultipleBytes(dev->client, dev->dev_7bits_i2c_addr, VD6283_CHANNELx_MM(chan),	values, 3);
	if (res)
		return res;
...
static STALS_ErrCode_t acknowledge_irq(struct VD6283_device *dev)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_IRQ_CTRL_ST, 1);
	if (res)
		return res;
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_IRQ_CTRL_ST, 0);
	if (res)
		return res;
...
static STALS_ErrCode_t dev_sw_reset(struct VD6283_device *dev)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_GLOBAL_RESET, 1);
	if (res)
		return res;
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_GLOBAL_RESET, 0);
	if (res)
		return res;
...
static STALS_ErrCode_t otp_reset(struct VD6283_device *dev)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OTP_CTRL1, 0);
	if (res)
		return res;

	do {
		res = STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OTP_STATUS, &status);
		if (res)
			return res;
...
static STALS_ErrCode_t opt_read_init(struct VD6283_device *dev)
{
	...
	res = STALS_RdMultipleBytes(dev->client, dev->dev_7bits_i2c_addr, VD6283_OTP_BANK_0, reg, 8);
	if (res)
		return res;
	...
	res = STALS_RdMultipleBytes(dev->client, dev->dev_7bits_i2c_addr, VD6283_OTP_BANK_1, reg, 8);
	if (res)
		return res;
...
static STALS_ErrCode_t otp_generate_uid(struct VD6283_device *dev)
{
	...
	res = otp_read(dev, 113, 3, &otp_data, 1);
	nible = otp_data << 1;
	res = (STALS_ErrCode_t) (res | otp_read(dev, 60, 1, &otp_data, 1));
	if (res)
		goto error;
	nible |= otp_data;
...
static STALS_ErrCode_t trim_oscillators(struct VD6283_device *dev)
{
	...
	/* 10.24Mhz */
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OSC10M, 1);
	if (res)
		return res;

	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OSC10M_TRIM_M,
		(hf_trim >> 8) & 0x01);
	if (res)
		return res;
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OSC10M_TRIM_L,
		(hf_trim >> 0) & 0xff);
	if (res)
		return res;

	/* 48.7 Khz */
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OSC50K_TRIM, lf_trim & 0x0f);
	if (res)
		return res;
...
static STALS_ErrCode_t clamp_enable(struct VD6283_device *dev)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_CLAMP_EN, 1);
	if (res)
		return res;
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_DC_CLAMP_EN, 0x1f);
	if (res)
		return res;
	/* set count to be independent of pd nummbers */
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_SPARE_1, 0x3f);
	if (res)
		return res;
...
static STALS_ErrCode_t select_pd(struct VD6283_device *dev)
{
	...
	for (c = 0; c < STALS_ALS_MAX_CHANNELS; c++) {
		res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_SEL_PD_x(c), pds[c]);
		if (res)
			return res;
	}
...
static STALS_ErrCode_t set_dithering(struct VD6283_device *dev)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OSC_DITHERING_STEP_VALUE, 1);
	if (res)
		return res;
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OSC_DITHERING_STEP_DURATION, 1);
	if (res)
		return res;
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OSC_DITHERING_STEP_NUMBER, 0x09);
	if (res)
		return res;
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_OSC_DITHERING_CTRL, 1);
	if (res)
		return res;
...
static STALS_ErrCode_t dev_configuration(struct VD6283_device *dev)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_SPARE_0,
...
static void put_device(struct VD6283_device *dev)
{
	...
	otp_reset(dev);
	STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_SPARE_0,
				VD6283_SPARE_0_IN_GPIO2);
	dev->st = DEV_FREE;
}
...
static STALS_ErrCode_t get_channel_gain(struct VD6283_device *dev, int c,	uint16_t *pAppliedGain)
{
...
static STALS_ErrCode_t set_channel_gain(struct VD6283_device *dev, int c,	uint16_t Gain)
{
	STALS_ErrCode_t res;
	uint8_t vref;
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_CHANNEL_VREF(c), vref + 1);
	if (res)
		return res;
...
static STALS_ErrCode_t get_exposure(struct VD6283_device *dev,	uint32_t *pAppliedExpoTimeUs)
{
...
static STALS_ErrCode_t set_exposure(struct VD6283_device *dev,	uint32_t ExpoTimeInUs, int is_cache_updated)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_EXPOSURE_L, exposure & 0xff);
	if (res)
		return res;
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_EXPOSURE_M,
		(exposure >> 8) & 0x3);
	if (res)
		return res;
...
static STALS_ErrCode_t dev_enable_channels_for_mode(struct VD6283_device *dev, enum STALS_Mode_t mode, uint8_t channels)
{
	...
	if (dc_chan_en ^ dev->dc_chan_en) {
		res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_DC_EN, dc_chan_en);
		if (res)
			return res;
	}
	if ((channels & VD6283_AC_CHANNELS_MASK) &&
	    !dev->ac_chan_en) {
		res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_EN, 1);
		if (res) {
			STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_DC_EN,
				dev->dc_chan_en);
			return res;
		}
		dev->ac_chan_en = 1;
	...
	enum STALS_Mode_t mode)
{
	...
	if (channels & VD6283_AC_CHANNELS_MASK) {
		res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_EN, 0);
		if (res) {
			STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_DC_EN,
				dev->dc_chan_en);
			return res;
...
static STALS_ErrCode_t enable_flicker_output_mode(struct VD6283_device *dev)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_INTR_CFG, intr_config);
	if (res)
		return res;

	/* interrupt output selection */
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_DTEST_SELECTION, sel_dig);
	if (res)
		return res;

	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_SPARE_0, spare0);
	if (res)
			return res;
...
static STALS_ErrCode_t disable_flicker_output_mode(struct VD6283_device *dev)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_INTR_CFG, 0x00);
	if (res)
		return res;

	/* and output als_complete signal */
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_DTEST_SELECTION, 0x00);
	if (res)
		return res;
	...
	enum STALS_Mode_t mode)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_ALS_CTRL, 0);
	if (res)
		return res;
	...
	enum STALS_Mode_t mode)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_ALS_CTRL, cmd);
	if (res)
		goto start_als_error;
	...
	uint32_t *value)
{
	...
	res = STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_PEDESTAL, &data);
	...
	uint32_t *value)
{
	...
	res = STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_SDA_DRIVE, &data);
	...
	uint32_t *value)
{
	...
	do {
		res = STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_SAT_METRIC_M, &sat_m_before);
		res = (STALS_ErrCode_t) (res | STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_SAT_METRIC_L, &sat_l));
		res = (STALS_ErrCode_t) (res | STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_SAT_METRIC_M, &sat_m_after));
		if (res)
			return res;
...
static STALS_ErrCode_t set_pedestal_value(struct VD6283_device *dev,
	uint32_t value)
{
	return STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_AC_PEDESTAL,
	...
	uint32_t value)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_SEL_PD_x(1), pd_value);
	if (res)
		return res;
	...
	uint32_t value)
{
	...
	return STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_SDA_DRIVE, data);
}
	...
	uint32_t value)
{
 ...
 static void cook_values_slope(struct VD6283_device *dev, struct STALS_Als_t *pAlsValue)
 {
...
STALS_ErrCode_t STALS_Init(const char *UNUSED_P(pDeviceName), DevI2C * pClient, uint8_t dev_7bits_i2c_addr, void **pHandle)
{
	STALS_ErrCode_t res;
	...
	setup_device(dev, pClient, dev_7bits_i2c_addr, *pHandle);
	// no otp read yet
...
STALS_ErrCode_t STALS_SetInterMeasurementTime(void *pHandle,	uint32_t InterMeasurmentInUs, uint32_t *pAppliedInterMeasurmentInUs)
{
	...
	res = STALS_WrByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_CONTINUOUS_PERIOD, value);
	if (res)
		return res;
...
STALS_ErrCode_t STALS_GetInterMeasurementTime(void *pHandle,	uint32_t *pAppliedInterMeasurmentInUs)
{
	...
	res = STALS_RdByte(dev->client, dev->dev_7bits_i2c_addr, VD6283_CONTINUOUS_PERIOD, &value);
	if (res)
		return res;
...
/*
STALS_ErrCode_t STALS_RdMultipleBytes(DevI2C * pClient, uint8_t dev_7bits_i2c_addr, uint8_t index,	uint8_t *data, int nb) __attribute__((weak));
STALS_ErrCode_t STALS_RdMultipleBytes(DevI2C * pClient, uint8_t dev_7bits_i2c_addr, uint8_t index,	uint8_t *data, int nb)
{
	...
	for (i = 0; i < nb; i++) {
		res = STALS_RdByte(pClient, dev_7bits_i2c_addr, index + i, &data[i]);
		if (res)
			return res;
	}
